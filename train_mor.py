...
def main():
    ...
    # Model configuration for ~400M parameters
    config = MoRConfig(
        vocab_size=50257,
        d_model=1280,
        n_heads=20,
    ...
    print(f"Model config: {config.d_model}d, {config.n_heads}h, {config.d_ff}ff, {config.n_layers_shared}L, {config.max_recursions}R")
    ...
    print(f"Training 400M MoR model for {max_iters} steps on {len(dataset)} batches...")
